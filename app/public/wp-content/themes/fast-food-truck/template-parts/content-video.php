...
<?php
  ...
  $fast_food_truck_video = false;

  // Only get video from the content if a playlist isn't present.
  if ( false === strpos( $fast_food_truck_content, 'wp-playlist-script' ) ) {
    $fast_food_truck_video = get_media_embedded_in_content( $fast_food_truck_content, array( 'video', 'object', 'embed', 'iframe' ) );
  }
?>
  ...
  <div class="box-image">
    <?php
      if ( ! is_single() ) {
        ...
        if ( ! empty( $fast_food_truck_video ) ) {
          foreach ( $fast_food_truck_video as $fast_food_truck_video_html ) {
            echo '<div class="entry-video">';
              echo $fast_food_truck_video_html;
            echo '</div>';
          }
        };
      }; 
